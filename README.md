...
По умолчанию приложение пытается использовать модель `llama3:8b`, запущенную на `http://localhost:11434` (это можно сделать через **Ollama**).
...
Оба скрипта корректно возвращают коды завершения приложения и подходят для использования в автозагрузке (systemd, cron `@reboot`, Windows Task Scheduler и т.д.).
