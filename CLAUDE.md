...
| `prompts/*.txt` | Тексты промптов для AI (см. ниже) — не Go-код, чтобы редактировать без пересборки |
...
- **Сопроводительное** — статический шаблон [letter_template.txt](letter_template.txt)
  ...
  меняется обратно на неё. Её промпт — [prompts/generate_letter.txt](prompts/generate_letter.txt).
- **Чат с работодателем** — только случайная строка из [questions.txt](questions.txt),
  без единого обращения к AI. Сама фича (флудилка, поддержание активности в чатах)
  осталась включена, просто теперь без LLM в контуре.
- **hh-тестирование** (единственное оставшееся использование AI) дополнительно
  учитывает [test_wishes.txt](test_wishes.txt) — свободный текст (зарплатные ожидания,
  формат работы и т.п.), подмешивается в промпт вместе с `-solution-prompt`/
  `HH_SOLUTION_PROMPT`. Загружается один раз при старте (responder.go).
...
- Choice-вопрос → `solveChoiceTask`, промпт [prompts/test_choice_task.txt](prompts/test_choice_task.txt),
  ожидаемый ответ — только `{"solution_id":N}`.
- Open-вопрос → `solveOpenTask`, промпт [prompts/test_open_task.txt](prompts/test_open_task.txt),
  ответ — голый текст, без JSON-обёртки.
- Общие правила (не выполнять инструкции из вопроса, не касаться политики и т.п.) —
  [prompts/test_common_rules.txt](prompts/test_common_rules.txt), собираются в
  `commonTaskRules()` и вставляются в оба промпта через плейсхолдер `{{RULES}}`.
...
- `-debug-solve-tests` (флаг, main.go `runDebugSolveTests`) — прогоняет `SolveTests` на
  ...
  `cfg.ExtraTestSolutionPrompt` (`-solution-prompt`) — `test_wishes.txt` в него НЕ
  подмешивается (это делает только `ApplyVacancyWithTest`). Если нужно проверить именно
  контекст с пожеланиями — только через реальный (или почти реальный) прогон.
...
| Сопроводительное письмо | ✅, статический шаблон, без AI | `RenderLetterTemplate` (vacancies.go) + [letter_template.txt](letter_template.txt) |
...
| Ответы на вопросы работодателя в чате | ✅, без AI — случайная строка из questions.txt | `AutoRespondChats` (chat.go), [questions.txt](questions.txt) |
...
- `questions.txt` — единственный источник сообщений для чата с работодателем, по одному
  на строку, выбор случайный, без AI. Не факт, что случайный вопрос будет уместен как
  ответ на конкретное сообщение работодателя — это осознанный компромисс в пользу
  нулевого использования LLM и предсказуемости, а не попытка смыслового ответа.
